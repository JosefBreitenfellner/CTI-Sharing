...
function connectToBlockchain(){
  ...
  if(status == true) {
    ...
    // address of smart contract (StoreData) --> value must be set after the deploy process
...
function chooseFile() {
    ...
    // if fileList contains any value store the name of the file in the variable fileName
    if(fileList[0] !== undefined) {
      fileName = fileList[0].name;
    }
    // else store an empty string
    else {
      fileName = "";
    }
}
...
/*
loadJSON is a function found in the internet from https://wiki.selfhtml.org/wiki/JSON
*/

function loadJSON(file,callback) {
  // new instance of an XMLHttpRequest
  ...
  xobj.onreadystatechange = function () {
    ...
    // readyState = 4 --> fetch operation is complete && status = 0 --> HTTP status code is unsent (just works with 0)
...
function loadJsonFile() {
  ...
  if(contractInstance == null) {
    ...
    loadJSON('./Cti-Files/' + fileName, function(text){
      // store all objects of CTI-Report in data
...
function displayCTI() {
  ...
  if(contractInstance == null) {
    ...
    contractInstance.methods.getArrayLength().call(function(err,res) {
      if(!err){
        // res is the length of the array 'objects'
        while(counter < res) {
...
function searchCTI() {
  ...
  for (var i = 0; i < tr.length; i++) {
    // description is the 4th column [3]
...
/*
downloadCsv and exportTableToCsv are functions found in the internet from https://codepen.io/malahovks/pen/gLxLWX
*/
...
function searchCti() {
  ...
  contractInstance.methods.searchObject(input).call(function(err,res){
    if(!err){
      var tdId = document.createElement("td");
    	tdId.innerHTML = res[0];
    	var tdCreated = document.createElement("td");
